#ifndef KOCS_PAIR_FINDERS_BINNED_GABRIEL_SYMMETRIC_HPP
#define KOCS_PAIR_FINDERS_BINNED_GABRIEL_SYMMETRIC_HPP
...
namespace kocs::pair_finders {
  ...
  /**
   * @brief Binned Gabriel-graph pair finder with symmetric force evaluation.
   *
   * Combines spatial binning with the Gabriel empty-ball criterion and
   * symmetric pairwise processing.  Particles are assigned to a regular grid
   * of bins, and for each particle @f$i@f$ only neighbouring bins are
   * searched.  Within those bins, only particles with index @f$j < i@f$
   * are considered, halving the number of pairwise evaluations.
   *
   * For each candidate pair within the cutoff, the Gabriel condition is
   * verified:  the scaled diametral ball of the pair must be empty of other
   * particles.  A second-level bin walk is performed around the midpoint of
   * the pair to efficiently test for blocking particles @f$k@f$.
   *
   * Force contributions are computed once and atomically applied to both
   * @f$i@f$ and @f$j@f$.  Drag and velocity corrections are accumulated
   * with atomics and normalised in a finalisation pass.
   *
   * This is the most performant Gabriel variant for mid-to-large systems
   * where the cutoff is small relative to the domain extent.
   *
  ...
  struct BinnedGabrielSymmetric {
    ...
    /**
     * @brief Constructs the binned symmetric Gabriel pair finder.
     * @param agent_count_     Number of particles.
     * @param cutoff_distance_ Interaction cutoff distance.
     * @param settings         Binning and Gabriel configuration.
     */
    BinnedGabrielSymmetric(
     ...
     *        binned spatial acceleration.
     *
     * The grid is rebuilt at the start of each full step (or according to
     * \c rebuild_every_n).  Each particle @f$i@f$ is processed by a team:
     * threads walk the neighbouring bins, and for each candidate @f$j < i@f$
     * within the cutoff, a second-level bin walk around the pair midpoint
     * efficiently checks the Gabriel condition.  If the condition is met,
     * the force functor is invoked once and deltas, drag, and velocity
     * corrections are atomically accumulated for both @f$i@f$ and @f$j@f$.
     * A final kernel normalises the drag correction.
     *
    ...
    void evaluate_force(
      ...
      Kokkos::View<Scalar*> total_drag("bg_sym_total_drag", agent_count);
      Kokkos::View<typename PositionsView::value_type*>
        total_velocity("bg_sym_total_velocity", agent_count);

      Kokkos::parallel_for(
        "binned_gabriel_symmetric_apply_force",
          ...
          Kokkos::parallel_for(
            ...
            [&](const int task_idx) {
              ...
              for (unsigned int idx = start; idx < end; ++idx) {
                ...
                // calculate exact overlapping bounds for the Gabriel ball
                VectorI min_bin;
                VectorI span;
                int task_count_2 = 1;
      ...
      Kokkos::parallel_for(
        "binned_gabriel_symmetric_apply_drag",
        agent_count,
...
#endif // KOCS_PAIR_FINDERS_BINNED_GABRIEL_SYMMETRIC_HPP
